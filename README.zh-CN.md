...
* **交付物管理与一键保存 (Artifacts)**：提供类似文件系统的目录树，分类管理 AI 产出的图表、代码、HTML、图片、PPT、报告等交付物（Artifacts），支持从聊天会话中一键保存并保留溯源。
...
* **Web 自动化测试 (Web Auto)**：基于 Playwright 的确定性浏览器自动化回归测试体系，具备两层断言评估（Node.js VM 沙箱表达式断言 + LLM 语义视觉评估），支持 SSE 实时运行进度流式广播。
* **断言验证与质量评估 (Verification & Evaluation)**：为 MCP 工具/工作流提供确定性输出断言测试，为 AI 对话代理提供基于 LLM-as-Judge 的对话质量评估。
