...
```bash
pnpm dev `credentials/crypto.ts`  `credentials/crypto.ts`  `credentials/crypto.ts`  `credentials/crypto.ts`  `credentials/crypto.ts`  `credentials/crypto.ts`  `credentials/crypto.ts` # Start dev server (Turbopack, port 3000)
pnpm build `credentials/crypto.ts`  `credentials/crypto.ts`  `credentials/crypto.ts`  `credentials/crypto.ts`  `credentials/crypto.ts`  `credentials/crypto.ts` # Production build
pnpm lint `credentials/crypto.ts`  `credentials/crypto.ts`  `credentials/crypto.ts`  `credentials/crypto.ts`  `credentials/crypto.ts`  `credentials/crypto.ts`  # ESLint check
pnpm db:generate `credentials/crypto.ts`  `credentials/crypto.ts`  `credentials/crypto.ts` # Generate Drizzle migration from schema changes
pnpm db:migrate `credentials/crypto.ts`  `credentials/crypto.ts`  `credentials/crypto.ts`  # Run pending migrations
pnpm db:push `credentials/crypto.ts`  `credentials/crypto.ts`  `credentials/crypto.ts`  `credentials/crypto.ts`  `credentials/crypto.ts` # Push schema directly (dev only)
pnpm db:studio `credentials/crypto.ts`  `credentials/crypto.ts`  `credentials/crypto.ts`  `credentials/crypto.ts` # Open Drizzle Studio
pnpm docker:db `credentials/crypto.ts`  `credentials/crypto.ts`  `credentials/crypto.ts`  `credentials/crypto.ts` # Start PostgreSQL via Docker Compose
```
