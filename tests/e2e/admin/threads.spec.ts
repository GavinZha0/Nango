...
test.describe("Thread Management", () => {
  test.beforeEach(async ({ page }) => {
    ...
    if (!page.url().includes("/admin/thread")) {
      test.skip(true, "Test user does not have admin access — first DB user was not our test user");
    }
    // Wait for the page content to load
    await expect(page.getByRole("heading", { name: "Threads" })).toBeVisible({ timeout: 10000 });
  });

  test("should display the threads page", async ({ page }) => {
