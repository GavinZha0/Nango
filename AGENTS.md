...
20. **Verification subsystem**: Deterministic assert-on-output testing framework exclusively for MCP tools. MCP test runs do not write `entity_run` records. Suite runs are serial, alphabetical, and fail-tolerant. Assertions support `json_schema`, `jsonpath`, and `js_expression` (vm sandbox). See [docs/verification.md](file:///d:/AI/nango/docs/verification.md).
