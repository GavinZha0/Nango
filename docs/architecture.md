...
**Parquet cache as the data-source ↔ sandbox handshake.** The two layers do not call each other directly. The data-source layer writes Parquet snapshots directly to `.cache/datasource/<name>/` via embedded `@duckdb/node-api`; the `dify-sandbox` layer mounts those files read-only into `/tmp/data` (exposed in-sandbox as `./tmp/data/<name>/`). This keeps the layers independently swappable and means the trust boundary is a file-system region, not an RPC interface.
