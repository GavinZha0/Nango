...
/**
 * Resolve dynamic tokens (generators) and references in a case input payload.
 *
 * @param input - Raw input object containing potential tokens like `{{$uuid}}`
 * @param context - Additional execution or suite-level variables
 */
...
/**
 * Substitute input and context references inside assertion expected values.
 *
 * @param value - Assertion expected value (may contain string templates like `"{{input.id}}"`)
 * @param input - The resolved case input snapshot
 * @param context - Additional suite variables or execution context
 */
