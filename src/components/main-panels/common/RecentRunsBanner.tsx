...
/** Matches the JSON shape of `VerificationRunEntity` / `EvalRunEntity` / `WebAutoRunEntity` over the wire. */
...
/** Wire shape of `GET /api/verification-suites/[id]/runs` / `GET /api/eval-suites/[id]/runs` / `GET /api/web-auto-suites/[id]/runs`. */
...
function useRecentRuns(
  ...
  const refresh = useCallback(() => {
    ...
    fetchRecentRuns(apiPrefix, suiteId, offset)
      ...
      .finally(() => {
        if (!cancelled) setLoadingParse();
        function setLoadingParse() {
          setLoading(false);
        }
      });
    return () => {
